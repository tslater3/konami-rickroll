(function() {
  ...
  function rickRoll() {
    ...
    (function() {
    	...
    	var maxNodes = 1000;
    	var animationCSS = {
    		'player_intro': 'playerIntro 1s infinite ease-in-out',
    		'rickRolled': function() {
    			var key = ~~ ( Math.random() * numTurntAnimations)
    			return 'rickRolled' + key + ' 5s infinite ease-in-out'
    		}
    	}
    	...
    	function checkTime() {
    		...
    		requestAnimationFrame(checkTime);
    		if(player.getCurrentTime() > rickRollAt) {
    			rickRolled = true;
    			removeCurStyles();
    			addCurStyles()
    		}
    	...
    	function setupAnimations() {
    		...
    		var introKeyFrameDef = '@-webkit-keyframes playerIntro { ' + introKeyFrames + ' } '
    		introKeyFrameDef += '@keyframes playerIntro { ' + introKeyFrames + ' } '
    		...
    		for(var i = 0; i < turntKeyFrames.length; i++) {
    			...
    			allStyles += '@-webkit-keyframes rickRolled' + i + ' { ' + kf + ' } '
    			allStyles += '@keyframes rickRolled' + i + ' { ' + kf + ' } '
    		}
    		var introClass = '.player_intro { -webkit-animation: player 1s infinite; animation: player 1s infinite; } '
    	...
    	function allClasses() {
    		return ['player_intro', 'rickRolled']
    	}
    	...
    	function init() {
    		if(typeof window.player________player !== 'undefined') {
    			return;
    		}
    		window.player________player = true;
